...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    public short readShort() {
        if (this.size >= 2) {
            ...
            int limit = segment.limit;
            if (limit - pos < 2) {
                return (short) (((readByte() & UByte.MAX_VALUE) << 8) | (readByte() & KotlinVersion.MAX_COMPONENT_VALUE));
            }
            byte[] data = segment.data;
            int pos2 = pos + 1;
            int pos3 = pos2 + 1;
            int s = ((data[pos] & UByte.MAX_VALUE) << 8) | (data[pos2] & KotlinVersion.MAX_COMPONENT_VALUE);
            this.size -= 2;
            if (pos3 == limit) {
                this.head = segment.pop();
                SegmentPool.recycle(segment);
    ...
    public int readInt() {
        if (this.size >= 4) {
            ...
            int pos5 = pos4 + 1;
            int i3 = i2 | (data[pos4] & KotlinVersion.MAX_COMPONENT_VALUE);
            this.size -= 4;
            if (pos5 == limit) {
                this.head = segment.pop();
                SegmentPool.recycle(segment);
            ...
            while (true) {
                ...
                if (triePos4 < 0) {
                    ...
                    while (true) {
                        ...
                        if ((data[pos] & KotlinVersion.MAX_COMPONENT_VALUE) != trie[triePos5]) {
                            return prefixIndex;
                        }
                    ...
                    int pos3 = pos + 1;
                    int b2 = data[pos] & KotlinVersion.MAX_COMPONENT_VALUE;
    ...
    public Buffer writeShort(int s) {
        ...
        int limit2 = limit + 1;
        data[limit] = (byte) ((s >>> 8) & KotlinVersion.MAX_COMPONENT_VALUE);
        data[limit2] = (byte) (s & KotlinVersion.MAX_COMPONENT_VALUE);
        tail.limit = limit2 + 1;
        this.size += 2;
        return this;
    }
    ...
    public Buffer writeInt(int i) {
        ...
        int limit = tail.limit;
        int limit2 = limit + 1;
        data[limit] = (byte) ((i >>> 24) & KotlinVersion.MAX_COMPONENT_VALUE);
        int limit3 = limit2 + 1;
        data[limit2] = (byte) ((i >>> 16) & KotlinVersion.MAX_COMPONENT_VALUE);
        int limit4 = limit3 + 1;
        data[limit3] = (byte) ((i >>> 8) & KotlinVersion.MAX_COMPONENT_VALUE);
        data[limit4] = (byte) (i & KotlinVersion.MAX_COMPONENT_VALUE);
        tail.limit = limit4 + 1;
        this.size += 4;
        return this;
    }
        ...
        public final long expandBuffer(int minByteCount) {
            if (minByteCount <= 0) {
                ...
                if (buffer2 == null) {
                    ...
                    tail.limit = ConstantsKt.DEFAULT_BUFFER_SIZE;
                    this.buffer.size = ((long) result) + oldSize;
                    this.segment = tail;
                    this.offset = oldSize;
                    this.data = tail.data;
                    this.start = 8192 - result;
                    this.end = ConstantsKt.DEFAULT_BUFFER_SIZE;
                    return (long) result;
                } else {
