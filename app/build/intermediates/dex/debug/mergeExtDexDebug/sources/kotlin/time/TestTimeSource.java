...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1382plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1346toLongimpl(duration, getUnit());
        if (longDelta == Long.MIN_VALUE || longDelta == LongCompanionObject.MAX_VALUE) {
            double newReading2 = ((double) this.reading) + Duration.m1343toDoubleimpl(duration, getUnit());
            if (newReading2 > ((double) LongCompanionObject.MAX_VALUE) || newReading2 < ((double) Long.MIN_VALUE)) {
                m1381overflowLRDsOJo(duration);
            }
    ...
    private final void m1381overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + Duration.m1349toStringimpl(duration) + '.');
    }
}
