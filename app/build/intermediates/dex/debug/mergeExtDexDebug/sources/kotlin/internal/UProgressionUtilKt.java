    ...
    public static final int m1200getProgressionLastElementNkh28Cs(int start, int end, int step) {
        if (step > 0) {
            if (UnsignedKt.uintCompare(start, end) < 0) {
                return UInt.m99constructorimpl(end - m1197differenceModuloWZ9TVnA(end, start, UInt.m99constructorimpl(step)));
            }
        } else if (step >= 0) {
            throw new IllegalArgumentException("Step is zero.");
        } else if (UnsignedKt.uintCompare(start, end) > 0) {
            return UInt.m99constructorimpl(m1197differenceModuloWZ9TVnA(start, end, UInt.m99constructorimpl(-step)) + end);
        }
        return end;
    }
    ...
    public static final long m1199getProgressionLastElement7ftBX0g(long start, long end, long step) {
        if (step > 0) {
            if (UnsignedKt.ulongCompare(start, end) < 0) {
                return ULong.m177constructorimpl(end - m1198differenceModulosambcqE(end, start, ULong.m177constructorimpl(step)));
            }
        } else if (step >= 0) {
            throw new IllegalArgumentException("Step is zero.");
        } else if (UnsignedKt.ulongCompare(start, end) > 0) {
            return ULong.m177constructorimpl(m1198differenceModulosambcqE(start, end, ULong.m177constructorimpl(-step)) + end);
        }
        return end;
    }
}
