...
class Huffman {
    ...
    public int encodedLength(ByteString bytes) {
        ...
        for (int i = 0; i < bytes.size(); i++) {
            len += (long) CODE_LENGTHS[bytes.getByte(i) & KotlinVersion.MAX_COMPONENT_VALUE];
        }
        return (int) ((7 + len) >> 3);
    }
    ...
    public byte[] decode(byte[] buf) {
        ...
        int nbits = 0;
        for (byte b : buf) {
            current = (current << 8) | (b & KotlinVersion.MAX_COMPONENT_VALUE);
            nbits += 8;
            while (nbits >= 8) {
                node = node.children[(current >>> (nbits - 8)) & KotlinVersion.MAX_COMPONENT_VALUE];
        ...
        while (nbits > 0) {
            Node node2 = node.children[(current << (8 - nbits)) & KotlinVersion.MAX_COMPONENT_VALUE];
    ...
    private void addCode(int sym, int code, byte len) {
        ...
        while (len > 8) {
            len = (byte) (len - 8);
            int i = (code >>> len) & KotlinVersion.MAX_COMPONENT_VALUE;
        ...
        int shift = 8 - len;
        int start = (code << shift) & KotlinVersion.MAX_COMPONENT_VALUE;
        int end = 1 << shift;
