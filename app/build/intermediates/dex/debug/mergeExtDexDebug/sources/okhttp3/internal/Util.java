...
public final class Util {
    ...
    public static final Comparator<String> NATURAL_ORDER = $$Lambda$Util$Ds7dtVnGrflEw4LvNOxA0cDT4Y.INSTANCE;
    ...
    private static InetAddress decodeIpv6(String input, int pos, int limit) {
        ...
        while (true) {
            if (i >= limit) {
                ...
                int b2 = b + 1;
                address[b] = (byte) ((value >>> 8) & KotlinVersion.MAX_COMPONENT_VALUE);
                b = b2 + 1;
                address[b2] = (byte) (value & KotlinVersion.MAX_COMPONENT_VALUE);
            }
        }
        if (b != address.length) {
            if (compress == -1) {
    ...
    private static String inet6AddressToAscii(byte[] address) {
        ...
        while (i2 < address.length) {
            if (i2 == longestRunOffset) {
                ...
                result.writeHexadecimalUnsignedLong((long) (((address[i2] & UByte.MAX_VALUE) << 8) | (address[i2 + 1] & KotlinVersion.MAX_COMPONENT_VALUE)));
                i2 += 2;
            }
        }
