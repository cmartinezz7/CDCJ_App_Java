...
public class JsonReader implements Closeable {
    ...
    private final char[] buffer = new char[LockFreeTaskQueueCore.MIN_ADD_SPIN_CAPACITY];
    ...
    private boolean isLiteral(char c) throws IOException {
        if (c == 9 || c == 10 || c == 12 || c == PEEKED_DOUBLE_QUOTED_NAME || c == ' ') {
            return false;
        }
        if (c != '#') {
            if (c == ',') {
    ...
    public void skipValue() throws IOException {
        ...
        do {
            ...
            if (p == 3) {
                ...
                this.stackSize--;
                count--;
            } else if (p == PEEKED_UNQUOTED_NAME || p == 10) {
                skipUnquotedValue();
            } else if (p == 8 || p == 12) {
                skipQuotedValue('\'');
            } else if (p == 9 || p == PEEKED_DOUBLE_QUOTED_NAME) {
    ...
    private int nextNonWhitespace(boolean throwOnEof) throws IOException {
        ...
        while (true) {
            ...
            if (p3 == 10) {
                this.lineNumber++;
                this.lineStart = p2;
            } else if (!(p3 == ' ' || p3 == PEEKED_DOUBLE_QUOTED_NAME || p3 == 9)) {
                if (p3 == '/') {
                    this.pos = p2;
                    if (p2 == l) {
                        this.pos = p2 - 1;
