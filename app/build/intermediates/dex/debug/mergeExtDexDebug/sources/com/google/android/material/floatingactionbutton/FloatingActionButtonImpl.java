...
class FloatingActionButtonImpl {
    ...
    public void calculateImageMatrixFromScale(float scale, Matrix matrix) {
        ...
        if (drawable != null && this.maxImageSize != 0) {
            ...
            drawableBounds.set(HIDE_OPACITY, HIDE_OPACITY, (float) drawable.getIntrinsicWidth(), (float) drawable.getIntrinsicHeight());
            int i = this.maxImageSize;
            imageBounds.set(HIDE_OPACITY, HIDE_OPACITY, (float) i, (float) i);
    ...
    public void hide(final InternalVisibilityChangedListener listener, final boolean fromUser) {
        ...
        if (!isOrWillBeHidden()) {
            ...
            if (shouldAnimateVisibilityChange()) {
                ...
                if (motionSpec != null) {
                    set = createAnimator(motionSpec, HIDE_OPACITY, HIDE_OPACITY, HIDE_OPACITY);
                } else {
                    set = createDefaultAnimator(HIDE_OPACITY, 0.4f, 0.4f);
                }
    ...
    private AnimatorSet createDefaultAnimator(float targetOpacity, float targetScale, float targetIconScale) {
        AnimatorSet set = new AnimatorSet();
        List<Animator> animators = new ArrayList<>();
        ValueAnimator animator = ValueAnimator.ofFloat(new float[]{HIDE_OPACITY, 1.0f});
        ...
        AnonymousClass4 r0 = new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                FloatingActionButtonImpl.this.view.setAlpha(AnimationUtils.lerp(f, f2, FloatingActionButtonImpl.HIDE_OPACITY, 0.2f, progress));
    ...
    private ValueAnimator createElevationAnimator(ShadowAnimatorImpl impl) {
        ...
        animator.setFloatValues(new float[]{HIDE_OPACITY, 1.0f});
        return animator;
    }
