...
final class ColorUtils {
    ...
    public static int blueFromInt(int argb) {
        return argb & KotlinVersion.MAX_COMPONENT_VALUE;
    }
    ...
    public static int intFromRgb(int r, int g, int b) {
        return (((((r & KotlinVersion.MAX_COMPONENT_VALUE) << 16) | -16777216) | ((g & KotlinVersion.MAX_COMPONENT_VALUE) << 8)) | (b & KotlinVersion.MAX_COMPONENT_VALUE)) >>> 0;
    }
    ...
    public static int intFromXyzComponents(float x, float y, float z) {
        ...
        float y2 = y / 100.0f;
        float z2 = z / 100.0f;
        return intFromRgb(Math.max(Math.min(KotlinVersion.MAX_COMPONENT_VALUE, Math.round(delinearized((3.2406f * x2) + (-1.5372f * y2) + (-0.4986f * z2)) * 255.0f)), 0), Math.max(Math.min(KotlinVersion.MAX_COMPONENT_VALUE, Math.round(delinearized((-0.9689f * x2) + (1.8758f * y2) + (0.0415f * z2)) * 255.0f)), 0), Math.max(Math.min(KotlinVersion.MAX_COMPONENT_VALUE, Math.round(255.0f * delinearized((0.0557f * x2) + (-0.204f * y2) + (1.057f * z2)))), 0));
    }
